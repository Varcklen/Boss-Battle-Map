scope SealOfMana initializer init

	globals
		private constant integer ITEM_ID = 'I08I'
		
		private constant integer STAT_INCREASE = 10
		private constant integer STAT_TYPE = STAT_MANA_HEAL_BONUS
		
		private constant integer STRING_HASH = StringHash( "seal_of_mana" )
	endglobals
	...
	private function check takes nothing returns nothing 
	    ...
	    local item itemUsed = LoadItemHandle( udg_hash, id, StringHash( "seal_of_mana_item" ) )
	...
	private function action takes nothing returns nothing 
		...
		set id = InvokeTimerWithItem( GetManipulatedItem(), "seal_of_mana_item", 1, true, function check )
