library BuffDeleteLib requires SpdLibLib, ShamanBallLib, MajyteLib, LuckLib, Multiboard, UniquesLib, StatLib, PauseLib, JesterLib
    ...
    function DelBuff takes unit u, boolean l returns nothing
        ...
        if GetUnitAbilityLevel(u, 'B09F') > 0 then
            call UnitRemoveAbility( u, 'A17P' )
            call UnitRemoveAbility( u, 'A17N' )
            call UnitRemoveAbility( u, 'B09F' )
        endif
        ...
        if GetUnitAbilityLevel( u, 'B08S' ) > 0 then
            call UnitRemoveAbility( u, 'A02Y' )
            call UnitRemoveAbility( u, 'B08S' )
            call SaveInteger( udg_hash, GetHandleId( u ), StringHash( "snkes" ), 0 )
        endif
